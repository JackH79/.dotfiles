...
class CustomApplications(Applications):
	def app_default(self, c):
		...
		if f.extension is not None:
			if f.extension in ('pdf'):
				c.flags += 'd'
				return self.either(c, 'zathura', 'apvlv', 'epdfview')
			if f.extension in ('html', 'htm', 'xhtml', 'swf'):
				return self.either(c, 'firefox', 'elinks')
			if f.extension in ('odt', 'doc'):
				return self.app_soffice(c)
			if f.extension in ('ods', 'xls'):
				return self.app_sofficec(c)
			if f.extension in ('odp', 'ppt'):
				return self.app_sofficei(c)
			if f.extension in ('tex'):
				return self.either(c, 'vim', 'texmaker')
			if f.extension in ('exe'):
				return self.either(c, 'wine')
		...
		if f.video or f.audio:
			if f.video:
				c.flags += 'd'
			return self.either(c, 'mplayer', 'vlc')

		if f.image:
			return self.either(c, 'mirage', 'geeqie', 'feh')
	...
	@depends_on("texmaker")
	def app_texmaker(self, c):
		c.flags += 'd'
		return tup('texmaker', *c)
	...
	@depends_on('sofficec')
	def app_sofficec(self, c):
		c.flags += 'd'
	#	os.environ["GTK2_RC_FILES"] = "/usr/share/themes/Clearlooks/gtk-2.0/gtkrc"
		return tup("soffice", "-calc", *c)
	
	@depends_on('sofficei')
	def app_sofficei(self, c):
		c.flags += 'd'
	#	os.environ["GTK2_RC_FILES"] = "/usr/share/themes/Clearlooks/gtk-2.0/gtkrc"
		return tup("soffice", "-impress", *c)
	...
	@depends_on('epdfview')
	def app_epdfview(self, c):
		return tup("epdfview", *c)
